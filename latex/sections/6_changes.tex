...
This section analyzes possibilities to allow for more flexibility in shaping \drivel{} traffic. Avoiding fingerprinting based on certain sizes, directionalities and timing of messages between client and bridge is crucial to evade censoring.

No changes discussed or proposed in this \cref{ssec:fragmentation} were implemented, or practically evaluated as part of this thesis. However, we believe that this discussion will be valuable to readers to understand possible future directions of the protocol.
...
As is obvious from the table, sending the entire first \drivel{} message in full before the bridge responds (after verifying the PRF value at the end of message) leads to a large quiet period. Thus, for all but a few KEM/OKEM combinations, this quiet period would quickly be identifiable. When trying to minimize the size of the first \drivel{} message, using Classic McEliece or ML-Kemeleon as an OKEM while employing ML-KEM or BIKE as a KEM would be close to optimal and yield sizes not exceeding 8~192 bytes. However, even these combinations would entail larger quiet periods than \obfsfour{} which would be easy to distinguish.
...
Violating a) would make the bridge vulnerable to probing attacks by censors that do not know the bridge information. On the other hand, violating b) would trivially break $\sObf$. Both of these outcomes are undesirable and $\sObfKE$ from \cite{CCS:GunSteVei24} captures both avenues of attack. This is because the predicate \textsf{Probed} automatically wins the game, and public keys can be revealed when asked to distinguish a simulated transcript from a real transcript in \textsc{ChallExec}, respectively.

Due to the required proof in the client's initial communication alone, it is strictly necessary that the bridge server receives a certain minimum amount of data before it responds in any way.
...
In order to outline the above requirements a bit more, we can informally define the initial interaction between an adversary and the bridge servers. One goal of the initial exchange, among others, is to establish a shared secret, that can aid in subsequent fragmentation, using just the first message from the client to the server.
...
Let $M$ denote the fixed-length message sent from the client to the bridge, and let $S$ denote the shared secret computed by both the client and the bridge for secure transport of subsequent traffic.
...
\begin{itemize}
    ...
    \item The adversary $\adv$ should not be able to distinguish a randomly sampled shared secret $S$ from a secret corresponding to any given message $M$ observed on the network, even if the adversary is allowed further communication with the bridge. Viewing the exchange as a KEM, this corresponds to $\indcca$ security.
...
The existence of a more efficient protocol would thus imply the existence of a more efficient OKEM, which, due to the large scope of the NIST Standardization~\cite{nist-standardization}, seems unlikely.
...
No changes discussed or proposed in this \cref{ssec:drivel-mod} were implemented, or practically evaluated as part of this thesis. However, we believe that this discussion will be valuable to readers to understand possible future directions of the protocol.
...
There is no need to check for repetitions between $\mathsf{MAC}_C$ values in this variant. The goal of these checks is to prevent the server from answering on replayed messages, in order to resist active probing.
...
Each set of $K_\mathsf{AE}, \mathsf{pre}, K_\mathsf{H}, \mathsf{IV}$ corresponds to either the client-to-bridge direction or vice versa. Both directions work in the same way presented below and visualized in \cref{fig:framing}.
...
This contrasts harshly with the handling of errors and invalid MAC values in the handshake procedure: At the start of a connection, the server chooses a random delay between 1 and 60 seconds as a deadline. If a client causes an error in the handshake or does not send enough data in the first 30 seconds, the connection is closed only at this randomized deadline, and not immediately upon the occurrence of the problematic behavior.
...
Conversely, this rather large change would require adapting the extensive $\sObfKE$ security proof for \drivel{} presented in \cite{EPRINT:GRSV25}.
...
For this purpose, we introduce a hash function $\mathbf{H}$ and replace the inputs to $F_1$ appropriately. This approach is similar in spirit to e.g. the calculation of the transcript hash in TLS 1.3 by keeping a running hash value \cite[Section~4.4.1]{rfc8446}.
