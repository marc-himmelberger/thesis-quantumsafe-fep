...
The growing spread of well-researched and high-performing cryptographic algorithms has enabled the development of many digital services, which are considered secure, such as electronic banking, messengers, digital identities, and digital signing, to name a few.
...
In order to provide the necessary anonymity to avoid censorship based on destination address, services such as the Tor Project \cite{torproject} rely on a dynamic set of entry nodes into the Tor network. These entry nodes, as well as all other nodes of the Tor network are primarily volunteer-operated, and together implement onion routing in order to anonymize users' traffic.

Nonetheless, the Tor protocol itself is easily identified due to e.g. lists of Tor entry nodes such as \cite{danNodeList} or using time-based characteristics \cite{icissp17}. Tor traffic can thus be blocked using relatively inexpensive traffic inspection. To remedy this issue, special protocols (so-called ``pluggable transports'') are required that are more difficult to identify. These protocols can then be used to communicate with the Tor network. In such a setup, so-called ``Tor bridges'' may be employed that replace typical entry nodes into the Tor network (``guards'') and IPs of bridges may be unknown to censors. By having Tor bridges run outside of censored regions, the pluggable transport only has to be used between the client and bridge.
...
To address \obfsfour{}'s reliance on traditional cryptography alone, the protocol \pqobfs{}~\cite{CCS:GunSteVei24} was proposed. Among other improvements, it allows the integration of these newly standardized KEMs.
Building upon \pqobfs{}, the \drivel{}~\cite{EPRINT:GRSV25} protocol further modifies the construction by lowering the requirements on the KEMs, and strengthening the security guarantees in the case of failures of some components.

\section{Our Contributions} \label{sec:contributions}

These proposed protocols have not yet been implemented in practice, but constitute a major change compared to \obfsfour{}.
In particular, performance profiles and bandwidth requirements are affected, and there is currently no concrete analysis of these effects on an end-to-end Tor connection.

This thesis therefore involves implementing the proposed \drivel{} construction from \cite{EPRINT:GRSV25} in order to test its practicality and performance.
...
In \cref{ch:experiment}, we evaluate the performance of our implementation using benchmarks and local, virtualized deployments. We quantify the tradeoffs between our suggested configurations and conclude that while our implementation integrates nicely into the existing lyrebird repository~\cite{lyrebird}, which also implements \obfsfour{}, and adds crucial flexibility to the protocol, some parts of the implementation could profit from further optimization.

\Cref{ch:protocol-changes} presents our proposal for concrete modifications to the \drivel{} specification of \cite{EPRINT:GRSV25}. We believe that a closer integration with the mechanisms used in the data transfer phase could benefit the key exchange while improving the potential of \drivel{} to hide within specific traffic patterns.

Finally, \cref{ch:conclusion} summarizes our findings and discusses possible directions for future work.
...
\begin{itemize}
    ...
    \item Security notions for the data transfer phase of FEPs were defined by Fenske and Johnson~\cite{CCS:FenJoh24}. Their work offers important considerations on how the encryption of higher-level data should be implemented for the data transfer phase and discusses possible pitfalls. The security notions capture the goals of encrypting or obfuscating all protocol data, and thus hiding protocol behavior from an adversary.
