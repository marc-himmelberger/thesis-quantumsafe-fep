...
The implementation created as part of this thesis is available via GitLab. It consists of two parts: A fork of the lyrebird project \cite{lyrebird} where the \drivel{} pluggable transport is implemented\footnote{See \url{https://github.com/marc-himmelberger/lyrebird-drivel}}, and a larger repository which includes a testing and benchmarking framework, analysis code and TeX sources of this document\footnote{See \url{https://github.com/marc-himmelberger/thesis-quantumsafe-fep}}.
...
In addition, it is possible to use the implementation of X25519 and \textsf{Elligator2}~\cite{CCS:BHKL13} as an OKEM, but as this OKEM would not be resistant against attacks carried out by quantum computers, we only include it for the purposes of performance comparisons with \obfsfour{}.
...
Similarly, but motivated also by the desire for a smaller traffic volume during the key exchange and a desire for smaller buffer sizes, Classic McEliece has been disabled as a KEM (though it remains available as an OKEM).
...
Although \drivel{} can be instantiated with any choice of KEM or OKEM listed above, most combinations lead to inconsistent security levels\footnote{For example, it may not make sense to pair an OKEM achieving NIST security level 5 with a KEM that only targets level 1. Given that overall security is then only level 1, one could replace the OKEM with more efficient parameter sets.} or excessive message sizes. To reduce the number of combinations to a sensible amount, \cref{tab:drivel-params} defines a total of 12 sets of KEM/OKEM combinations, four for each of the NIST security levels 1, 3, and 5.
...
Further, we did not use sntrup761 as a KEM (even though it would offer a 30\% reduction in size compared to BIKE-L1) because no other NTRU Prime parameter sets are available for NIST levels 3 and 5. Instead we wanted to provide similar KEM/OKEM combinations for all targeted NIST levels.
...
Readers should note that, although the construction is similar in spirit to the DHKEM construction \cite[Section~4.1]{rfc9180}, this KEM has minor differences in the derivation of its shared secret. The focus was on staying as close as possible to the construction used in \obfsfour{}, in order to offer a fair comparison between the two pluggable transport constructions when targeting traditional security.
...
Due to the different origin of these KEM and OKEM implementations as well as the lack of an abstract standardized KEM interface in Go suitable for these constructions, we added several new submodules to \texttt{internal}:
\begin{itemize}
...
While \texttt{transports/obfs4} depends on \texttt{common/ntor} to implement the main cryptography beyond obfuscation, this functionality is now provided instead by \texttt{transports/drivel/drivelcrypto}. This functionality is now less distributed across the repository. Test cases from the \texttt{common/ntor} module were copied and extended as needed.
...
One noteworthy change in particular is that the sizes of public keys, private keys, ciphertexts and shared secrets are not statically known anymore, as the schemes are only selected at runtime.
...
As a final change, we modified \texttt{transports/drivel/drivel.go} and \texttt{transports/drivel/statefile.go} which handle the high-level protocol functionality such as parsing arguments and loading saved bridge information, as well as constructing the connection used for later data transfer after the handshake phase is completed.
...
\begin{enumerate}
    \item Clone our fork of the lyrebird repository\footnote{See \url{https://github.com/marc-himmelberger/lyrebird-drivel}} into a new directory called ``lyrebird''
    ...
    \item Adapt the files of ``docker-obfs4-bridge'' to match the contents of the corresponding files in the \texttt{docker/bridge} folder in our analysis repository\footnote{See \url{https://github.com/marc-himmelberger/thesis-quantumsafe-fep}}. This changes the build process to not use the official lyrebird repository but instead copy the contents of a local ``lyrebird'' folder, install all necessary build dependencies and perform the build. Additionally, the bridge-line format requires minor changes to the ``get-bridge-line'' and ``start-tor.sh'' files.
