...
Due to the fact that our implementation relies on AES anyway, and to reduce the amount of options to consider in the following sections, we support only AES variants of FrodoKEM.
Similarly, but motivated also by a desire for a smaller traffic volume during the key exchange and a desire for smaller buffer sizes, Classic McEliece has been disabled as a KEM (though remains available as an OKEM).
...
Note that some ``FEO-'' schemes may in fact simply be the unmodified KEM (the code uses a \texttt{nil} encoder reference), but the naming convention ensures that the name of a scheme clearly separates KEMs from OKEMs. Additionally, as discussed above in \cref{ssec:drivel-instance}, some KEMs and OKEMs available from \texttt{liboqs} are disabled to limit the number of test cases and enable some optimizations with regards to buffer sizes.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{internal/cryptofactory/oqs\_wrapper} implements a thin wrapper which makes the \texttt{liboqs} KEM interface compatible with our interface defined in \texttt{internal/kems}.
    \end{itemize}
\end{itemize}
...
The main protocol functionality is implemented in \texttt{transports/drivel/handshake.go}, handling state objects persisted through the course of a connection, and imlementing functions to initialize this state, parse messages, and generate responses.
...
One noteworthy change in particular is that the sizes of public keys, private keys, ciphertexts and shared secrets is not statically known anymore due to the added crypto-agility. While we could have hardcoded these values for slight performance gains, we believe it to be more prudent to respect sizes reported by \texttt{liboqs} and limit the changes required to switch out a KEM or OKEM to a minimum.
...
\begin{itemize}
    ...
    \item create a file called ``drivel-key-KEYID'' containing $\pk_S$ in JSON format, where ``KEYID'' is a prefix of the $\mathsf{NodeID}$.
\end{itemize}

As the configuration is now vastly shorter, it can be transmitted via the SOCKS fields, and the pluggable transport can load the public key corresponding to the $\mathsf{NodeID}$ from disk.
...
Some modifications to the protocol may prove promising in the future, both in terms of flexibility and performance, but also for obfuscation. Nonetheless, we believe this implementation to provide a valuable jumping-off point for future work.
...
\begin{itemize}
    ...
    \item There is a lack of generic client arguments to configure the pluggable transport beyond what is available through the SOCKS fields while servers can make use of existing parsing of the environment variable ``TOR\_PT\_SERVER\_TRANSPORT\_OPTIONS''. Such a mechanism could be used in the future to avoid recompilation when changing the employed KEM and OKEM combination. Whether the KEM and OKEM settings should be part of the bridge information is left up to the Tor Project, as this requires coordination between bridges and clients to achieve interoperability while respecting client's choice of trusted schemes.

    \item This thesis contains one of the first implementations of ML-Kemeleon. One previous implementation did not implement the \texttt{SamplePreimage} function from the Internet-Draft \cite{ct-kemeleon}, and another only had partial test cases for this function \cite{jmwample-kemeleon} and did not correctly sample uniformly random outputs. We implemented comprehensive tests, and verified the expected output distribution of \texttt{SamplePreimage}. Necessary changes to the Internet-Draft were identified in the process and the draft authors, as well as the authors of the two previous implementation were informed of our findings.
...
In order for software to remain flexible for future adaptations, it is beneficial to avoid hardcoding specific algorithms. A modular approach where components such as encryption, PRFs, and KEMs are easily exchangeable ensures that future developments cause fewer changes to the overall system. Even innocent-looking limitations such as the SOCKS field lengths can constitute integration barriers in the future.
