...
One protocol targeting exactly this property is the \obfsfour{}~\cite{obfs4} protocol.
In this thesis, we build on the Tor Project's lyrebird repository implementing \obfsfour{}. Later, the \obfsfour{} protocol was extended by the obfuscated key exchange protocol \drivel{}~\cite{EPRINT:GRSV25} with post-quantum security and stronger obfuscation.
We implement \drivel{} and integrate it into the lyrebird repository. As \drivel{} requires ciphertexts to look random, we analyze the Classic-McEliece and HQC key encapsulation mechanisms, defining mappings from ciphertexts to random byte strings. These schemes are then integrated with our implementation, in addition to the previously examined ML-KEM and FrodoKEM schemes.
Finally, we empirically evaluate our implementation using benchmarks and local deployments and suggest concrete changes to \drivel{} to reduce identifiable traffic patterns in the handshake and increase performance.
\end{abstract}
