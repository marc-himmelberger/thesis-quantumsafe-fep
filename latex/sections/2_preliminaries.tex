...
\begin{definition}[$\indcca$ and $\sprcca$ security] \label{def:kem-security}
...
\begin{theorem}[Encapsulate-then-Encode Security]
...
\begin{proof}
    ...
    To bound the difference between $p_1$ and $p'$, we reduce to the $\sprcca$ security of $\KEM$:
    Let $b$ be the bit from the $\sprcca$ game. Let $\bdv$ be a reduction which, given $\pk, c, K$, computes $\hat c \gets \encodectxt(c)$, aborts if $\hat c = \bot$, and if not, outputs $\adv(\pk, \hat c)$.
    Assuming $\bdv$ does not abort, this perfectly simulates either game $G_1$ (if $b=1$) or game $G'$ (if $b=0$) towards $\adv$.
    
    Let $\mathsf{ABRT}$ denote the event that $\bdv$ aborts, and let $b'$ be the output of $\adv$. Using advantage rewriting we can then express the advantage of $\bdv$ in the $\sprcca$ game as follows:
    \begin{align*}
    ...
    We conclude that
    \[
    \left| p_1 - p' \right| = 1/\epsilon^\firstencapssuccess_{\KEM,\encodectxt} \cdot \advantage{\sprcca}{\KEM, \simulunif}[(\bdv)].
    \]
