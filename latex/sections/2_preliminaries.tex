...
\begin{definition}[Obfuscated KEM]
    ...
    \end{itemize}
    Note that any OKEM, as defined here, can also be viewed as a KEM as in \cref{def:kem}. Crucially, however, the ciphertext space of an OKEM must a space of fixed-length bit strings.
...
For further information not reproduced in \cref{fig:drivel}, we refer the reader to the original publication.
...
Initially, clients are aware of the bridge information consisting of an OKEM public key $\pk_S$ and a random bit string, $\mathsf{NodeID}$.
...
After transmitting the KEM ciphertext, $\mathsf{auth}$ and two PRF values, the client verifies the authenticity of the message and of $\mathsf{auth}$ before also deriving the now securely shared value $\mathsf{KEY\_SEED}$.
